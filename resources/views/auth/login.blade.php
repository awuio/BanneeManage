<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" novalidate x-data="{ isSubmitting: false }" @submit="isSubmitting = true"
        class="space-y-4">
        @csrf
        ...
        <!-- Password -->
        <div class="space-y-1.5" x-data="{ show: false, capsOn: false }" @keydown.window="capsOn = $event.getModifierState('CapsLock')"
            @keyup.window="capsOn = $event.getModifierState('CapsLock')">
            ...
            <div class="relative">
                <x-text-input id="password" class="w-full pr-14" x-bind:type="show ? 'text' : 'password'"
                    name="password" required autocomplete="current-password" placeholder="••••••••" />

                <div class="absolute inset-y-0 right-0 flex items-center pr-3 gap-2">
                    <!-- Caps Lock Indicator -->
                    <div x-show="capsOn" style="display: none;" class="flex items-center text-zinc-600" title="Caps Lock is ON">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 19h6" />
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 15v-3H5l7-7 7 7h-4v3H9z" />
                        </svg>
                    </div>

                    <!-- Show/Hide Button -->
                    <button type="button" @click="show = !show"
                        class="text-zinc-400 hover:text-zinc-600 focus:outline-none flex items-center">
        ...
        <div class="flex items-center gap-2">
            <input id="remember_me" type="checkbox"
                class="h-4 w-4 rounded border-zinc-300 text-zinc-900 shadow-sm focus:ring-zinc-400" name="remember">
